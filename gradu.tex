...
Elliptical galaxies (Es) are part of the Hubble classification of galaxies \citep{Hubble1926}. They are so-called "early-type galaxies" (ETGs); meaning that, in the Hubble "tuning-fork" diagram, they are located left of the point where the diagram diverges into the two spiral galaxy paths, and thus do not contain spiral arms.

As their name implies, elliptical galaxies are quite ellipsoidal in shape. They contain constant surface brightness contours (isophotes) that can be described as co-centric ellipses, and lack a rapidly rotating disk. The stellar population of elliptical galaxies is generally quite old, as they contain either a small amount of cold gas, or essentially none at all. This lack of cold gas corresponds well with the galaxies' observed red colours and absence of star-formation \citep{BinneyTremaine, Cappellari2016}. 

In the Hubble classification, elliptical galaxies are further divided into seven different subcategories according to their ellipticity (E0 - E7; where the number denotes the tenth multiple of the galaxy's ellipticity rounded to the nearest integer). Ellipticity itself is simply the measure of how flattened an observed 2D-projection of a spherical stellar system is. It can be calculated using the equation:
\begin{equation}
\epsilon = 1 - \frac{b}{a},
\end{equation}
where $a$ and $b$ parameters are the semi-major and semi-minor axes of a luminosity isophote respectively. The larger the ellipticity, the flatter the system ($\epsilon = 0$ denotes a completely spherical galaxy). It is important to note, however, that the ellipticity of a system depends on the luminosity contour from which it is calculated. As the isophotes of elliptical galaxies generally become flatter the farther they are from its centre \citep{BinneyTremaine}, this could result in a single galaxy having multiple ellipticities. To remedy this, the Hubble classification uses the ellipticity at the effective radius ($R_e$), i.e. the radius which encloses half of the galaxy's luminosity, to determine the subcategory of an elliptical galaxy.

\subsection{Surface Brightness}

The study of extra-galactic objects consists solely of observations of the light they emit, much of the information about the physical structure of elliptical galaxies comes from the brightness distribution of the observed light. The quantity used for describing this distribution is usually the "surface brightness" ($I$), which is defined as the amount of luminosity observed from a unit area. It is important to use a two dimensional quantity instead of a three dimensional quantity to describe the distribution, since astronomical observations simply measure the properties of 2D projections of actual 3D objects.

In the past, it has been shown that the radial surface brightness profiles of elliptical galaxies can be modelled quite well with a single theoretical profile. \cite{deVaucouleurs1948} showed that 
