...
%Finding a positive correlation between the size of the core and the black hole binary mass is fundamental, when trying to provide evidence for the idea that core formation occurs as a result of SMBH mergers. Such a correlation has been detected in observations \citep{Thomas2016}. However, an advantage of using simulations is that, unlike in observations where the central SMBH mass has to be estimated from its surrounding stellar velocity distribution, the mass of the black hole is known explicitly. 

%The KETJU simulations produce results that contain an inherent link between the size of the core and the binary mass. With the exception of a few core radii derived using the Nuker-model fitting method, the core size clearly follows a positive correlation with the SMBH binary mass. These abnormally large core radii should not pose a problem for the final conclusion of this thesis. The Nuker-model has been shown to be dependent on when its fitting range, thus it should not be unexpected for it to give inconsistent results, when using a constant range from one Nuker-fit to another. Certainly, this does not invalidate the results gathered from the other two core size measurement results.
