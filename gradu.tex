...
We analyse the results from two sets of KETJU simulations, one done by \cite{Mannerkoski2019}, and another by \cite{Rantala2018}. Both sets simulate early type galaxy mergers, where the progenitors contain supermassive black holes with masses ranging from $8.5 \times 10^8 M_\odot$ to $8.5 \times 10^9 M_\odot$. They use the same progenitor galaxies in their simulations, however, where as \cite{Mannerkoski2019} use 
...
As stated before, in order to calculate the sizes of the cores from profiles such as the ones seen in figure \ref{figure:surface_brightness}, we need to fit some kind of theoretical model that can describe a profile with surface brightness deficiency near the galactic centre. There are two commonly used options for this. The first one is the core-Sérsic profile \citep{Graham2003}, which divides the single power-law Sérsic profile into a combination of an inner and an outer power-law. The profile can be expressed using the following equation:
\begin{equation}
...
\begin{figure}[h]
	...
	\caption{Core-Sérsic profile fits of the surface brightness data calculated from all of the individual simulated merger remnants with progenitors containing central supermassive black holes. The letters (a)-(f) denote the different merger remnants ((a): BH-1, (b): BH-2, (c): BH-3, (d): BH-4, (e): BH-5, (f): BH-6).}
\end{figure}
