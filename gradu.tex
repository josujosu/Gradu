...
\end{equation}
where $\rho(r)$ is the density profile from equation \ref{eq:dehnen_density}. Once the positions of the particles are known, their velocities can be also determined using the Eddington's formula \citep{BinneyTremaine}, which gives the following distribution function for the different particles in the position-velocity phase-space:
\begin{equation}
...
Table \ref{table:properties} also shows the mass of the central SMBH in each of the seven progenitor galaxies, which is the only physical property that changes from one progenitor to another. Six of the progenitor galaxies (BH-1 - BH-6) contain central supermassive black holes, with the SMBH masses varying from $8.5 \times 10^8 M_\odot$ to $8.5 \times 10^9 M_\odot$, where a merged binary of the largest SMBHs is equivalent in mass to the central SMBH in NGC 1600. The seventh progenitor (BH-0) does not contain an SMBH in its centre, and is included simply for the sake of comparison.

The simulations themselves comprise of seven merger runs of two identical progenitor galaxies from table \ref{table:properties}, where the galaxies are merged on a nearly parabolic orbit with an initial separation of $d = 30 \mathrm{kpc}$. According to \cite{Rantala2018}, this kind of orbit makes the approach of the galaxies swift, and causes the stellar cusps to merge before $t \sim 300 \mathrm{Myr}$.

The simulation data that I will be analysing, comes in the form of a snapshot of the merger remnant at the simulation time of $\sim 2 \mathrm{Gyr}$. At this point the progenitor galaxies have merged into a single merger remnant galaxy with a binary SMBH in the centre. These single snapshots contain the positions, velocities and masses of every particle; be they stellar particles, dark matter particles or black holes.
...
We calculate the core radii of the merger remnants by using the "Levenberg-Marquardt" fitting algorithm to fit both the core-Sérsic model and the Nuker model to the remnant's surface brightness profile. Figure \ref{figure:core_nuker} shows a comparison of the resulting fits for the BH-3 merger (refer to table \ref{table:properties}), while figures \ref{figure:all_core} and \ref{figure:all_nuker}, located in the appendix, show the fits for every remnant that contains SMBH binaries. The values of the best-fit parameters are written on the figures, and it is clear that the exact value of the best-fit break radius, i.e. the core radius estimate, depends quite strongly on the used model.
...
The size of the core being dependent on the mass of the SMBH binary is clear evidence towards the cores being formed through a scouring process by binary black holes. As the missing stellar particles, which cause the surface brightness deficiencies in the galaxies' light curves, have been ejected from the system through gravitational three-body interactions with the SMBH binary; 
...
\begin{figure}
	...
	\caption{Beta-profile of NGC 1600, alongside profiles of galaxies from the SINFONI black hole survey (references), and the range of anisotropies found in N-body simulations of the core scouring mechanism \citep{Thomas2016}.}
	\label{figure:beta_NGC1600}
\end{figure}
...
Figure \ref{figure:snapshot_0_and_1_IFU} shows the voronoi binned 2D maps of the four LOS velocity distribution parameters for Snapshot-0 and Snapshot-6. The contours denote the merger remnants' flux isophotes, and have a spacing of one magnitude. Similar maps for every merger remnant can be seen in figures \ref{figure:all_voronoi_1} and \ref{figure:all_voronoi_2} in the appendix.
...
\begin{table}
	\begin{center}
		...
		\hline
		Snapshot-6 & $8.3$ & $1.7$ & $8.914$ & $17.7$ & $4$ & $5.84$ & $311$ & $0.0215$ & $0.14$ \\
